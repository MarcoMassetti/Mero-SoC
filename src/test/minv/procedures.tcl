...
proc load_code {} {
	set input_file "$::env(OBJ_DIR)/minv.bin"
...
proc run_until_break {max_time} {
	...
	set trap_path  "sim:/cpu_top_tb/DUT/inst_cpu/trap_wb_r"
	set rf_path "sim:/cpu_top_tb/DUT/inst_cpu/inst_register_file/registers"
	
	when -label trap "$trap_path'EVENT and $trap_path=1 and $rf_path\(17)==a" {
		nowhen trap
		nowhen timeout
...
proc report_rf {} {
	set output_file "$::env(OBJ_DIR)/rf_dut.txt"
	set rf_path "sim:/cpu_top_tb/DUT/inst_cpu/inst_register_file/registers"
...
proc compare_rf_content {} {
	if {[catch {exec diff $::env(OBJ_DIR)/rf_dut.txt $::env(OBJ_DIR)/rf_golden.txt} fid]} {
